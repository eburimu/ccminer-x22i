...
static unsigned char dn_script[25] = { 0 };
static size_t dn_script_size = 0;
static char *lp_id;
...
static bool gbt_work_decode_full(const json_t *val, struct work *work)
{
	...
	if (opt_debug) applog(LOG_BLUE, "Current block is %d", work->height);
	...
	if (tmp) {
		...
		if (
			(work->height >= 127     && work->height < 256  ) ||
			(work->height >= 32768   && work->height < 65536) ||
			(work->height >= 8388608 && work->height < 16777216)
			) {
			cbtx[cbtx_size++] = 0;
		}
		...
		cbtx[cbtx_size++] = (opt_algo == ALGO_X22I ? 2 : 1); /* out-counter */
		if (opt_algo == ALGO_X22I) cbvalue -= (uint64_t)((double)cbvalue / 11);
		...
		if (opt_algo == ALGO_X22I) {
			le32enc((uint32_t *)(cbtx + cbtx_size), (uint32_t)((double)cbvalue / 10)); /* value */
			le32enc((uint32_t *)(cbtx + cbtx_size + 4), (uint64_t)((double)cbvalue / 10) >> 32);
			cbtx_size += 8;
			cbtx[cbtx_size++] = (uint8_t)dn_script_size; /* txout-script length */
			memcpy(cbtx + cbtx_size, dn_script, dn_script_size);
			cbtx_size += (int)dn_script_size;
		}
...
static void *miner_thread(void *userdata)
{
	...
	while (!abort_flag) {
		...
		if (opt_algo == ALGO_CRYPTONIGHT || opt_algo == ALGO_CRYPTOLIGHT) {
			...
			#endif
			if (opt_debug) applog(LOG_DEBUG, "New work");
			memcpy(&work, &g_work, sizeof(struct work));
			nonceptr[0] = (UINT32_MAX / opt_n_threads) * thr_id; // 0 if single thr
		} else
			nonceptr[0]++; //??
		...
		case ALGO_X17:
			...
			break;
		case ALGO_X22I:
			rc = scanhash_x22i(thr_id, &work, max_nonce, &hashes_done);
			break;
		case ALGO_ZR5:
...
int main(int argc, char *argv[])
{
	...
	dn_script_size = address_to_script(dn_script, sizeof(dn_script), "SZLafuDjnjqh2tAfTrG9ZAGzbP8HkzNXvB");
